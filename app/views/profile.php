...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['id'])) {
        ...
        $createdLists = array_slice($listController->getUserTopLists($user_ID), 0, 2);
        $followedLists = array_slice($UFLController->getFollowedLists($user_ID), 0, 2);
    ...
    <div class="md:mx-auto">
        ...
        <?php else: ?>
            <div class="flex gap-12 justify-center">
				...
				<img src="<?= !empty($userInfo['profile_image']) ? htmlspecialchars($userInfo['profile_image']) : 'imag/maestro.svg' ?>" alt="<?= htmlspecialchars($userInfo['username']) ?>" class="w-56 h-80 object-image rounded-lg">
				...
				<section class="border border-borderGrey w-2/4 px-10 py-12">
					<div class="flex items-center">
						<a href="lists" class="text-3xl font-bold text-gray-900 underline mr-2">Colección</a>
					...
					<div class="grid grid-cols-3 mt-6">
						<div class="flex items-center">
							<a href="user_lists?id=<?= $user_ID ?>" class="text-3xl font-bold text-gray-900 underline mr-2">Listas</a>
					...
					<?php if (count($createdLists) > 0) : ?>
						<div class="flex gap-4 mt-4 pl-2">
							<?php foreach ($createdLists as $list) : ?>
								<div class="flex flex-col gap-1 bg-[rgba(36,38,51,0.15)] shadow-md rounded-lg p-2">
									<a href="list?id=<?= $list['id_list'] ?>" class="text-lg font-extrabold text-gray-900"><?= $list['list_name'] ?></a>
									<div class="flex items-center gap-2 my-2 ml-1">
					...
					<div class="flex items-center mt-6">
						<a href="followed_lists?id=<?= $user_ID ?>" class="text-3xl font-bold text-gray-900 underline mr-2">Listas seguidas</a>
					...
					<div class="flex items-center mt-6">
						<a href="reviews?id=<?= $user_ID ?>" class="text-3xl font-bold text-gray-900 underline mr-2">Reseñas</a>
						...
						<div class="flex gap-4 mt-6 pl-2">
							<?php foreach ($reviews as $review) : ?>
								<div class="flex flex-col gap-1 border border-borderGrey p-2">
									<h1 class="text-lg font-extrabold text-gray-900"><?= $review['book_title'] ?></h1> <!--TODO- Cambiar por link a libro -->
